...
async def post_login(cookie: Optional[AuthCookie] = Depends(get_login_form_creds)) -> RedirectResponse:
  if cookie:
...
@router.get(**logout)
@router.post(**logout)
async def post_login(cookie: Optional[AuthCookie] = Depends(get_auth_cookie)) -> RedirectResponse:
  if not cookie:
